...
import { completeResetPassword, validateResetPasswordToken } from '../services/ysApi'
...
const pageMeta = computed(() => {
  if (isComplete.value) {
    return {
      ...
      title: t('password_flow.reset.complete_title'),
      description: t('password_flow.reset.complete_description'),
    }
  }
  ...
  if (!isLoading.value && !tokenState.value.valid) {
    return {
      ...
      title: tokenState.value.expired
        ? t('password_flow.reset.expired_title')
        : t('password_flow.reset.invalid_title'),
      description: t('password_flow.reset.invalid_description'),
    }
  }

  return {
    eyebrow: 'RESET PASSWORD',
    title: t('password_flow.reset.title'),
    description: t('password_flow.reset.description'),
  }
})
...
const validateToken = async () => {
  ...
  submitError.value = ''
  try {
    tokenState.value = await validateResetPasswordToken(token.value)
  } catch (error) {
    submitError.value = error?.message || t('password_flow.reset.errors.validate')
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    successMessage.value =
      ...
      result?.message ||
      t('password_flow.reset.success_message')
    isComplete.value = true
  } catch (error) {
    submitError.value = error?.message || t('password_flow.reset.errors.submit')
  } finally {
...
<template>
  ...
  >
    <div v-if="isLoading" class="password-action-state">
      <p>{{ t('password_flow.reset.loading') }}</p>
    </div>

    <div v-else-if="isComplete" class="password-action-state is-success">
      <div class="password-action-state__badge">✓</div>
      <p class="password-action-state__headline">{{ t('password_flow.reset.success_headline') }}</p>
    ...
    <div v-else-if="!tokenState.valid" class="password-action-state is-error">
      ...
      <p class="password-action-state__copy">
        {{ t('password_flow.reset.invalid_hint') }}
      </p>
      <button type="button" class="password-action-button is-primary" @click="goBackToLogin">
        {{ t('password_flow.common.back_to_login') }}
      </button>
    ...
    <form v-else class="password-action-form" @submit.prevent="handleSubmit">
      <p class="password-action-form__note">
        {{ t('password_flow.reset.account_label') }}:
      ...
      <div class="password-action-form__actions">
        ...
        <button type="submit" class="password-action-button is-primary" :disabled="!canSubmit">
          {{ isSubmitting ? t('common.actions.updating') : t('password_flow.reset.submit') }}
        </button>
      </div>
    </form>
  </PasswordActionShell>

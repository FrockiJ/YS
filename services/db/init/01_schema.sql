...
CREATE TABLE IF NOT EXISTS entities(
  ...
  type TEXT, value TEXT, normalized TEXT,
  confidence REAL, meta JSONB DEFAULT '{}'::jsonb,
  created_at TIMESTAMP DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS alias(
  ...
  type TEXT, lang TEXT, source TEXT,
  confidence REAL, created_at TIMESTAMP DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS errors(
  id BIGSERIAL PRIMARY KEY,
  document_id INTEGER, chunk_id BIGINT,
  stage TEXT, field TEXT, reason TEXT, detail JSONB,
  created_at TIMESTAMP DEFAULT now()
);
